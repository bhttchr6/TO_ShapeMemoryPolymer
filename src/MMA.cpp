...
//VecView(dgdx[0], PETSC_VIEWER_STDOUT_WORLD);
//VecView(dgdx[0], PETSC_VIEWER_STDOUT_WORLD);
//VecView(dgdx[1], PETSC_VIEWER_STDOUT_WORLD);
	if (!NonLinConstraints)	{
...
//VecView(xval, PETSC_VIEWER_STDOUT_WORLD);
	// Update xolds
	VecCopy(xo1,xo2);
	VecCopy(xval,xo1);

	// Solve the dual with an interior point method
//PetscPrintf(PETSC_COMM_WORLD, "solving DIP");
	SolveDIP(xval);
//PetscPrintf(PETSC_COMM_WORLD, "DIP solved");
//VecView(xval, PETSC_VIEWER_STDOUT_WORLD);
	return ierr;
}
...
//VecView(x, PETSC_VIEWER_STDOUT_WORLD);
	PetscErrorCode ierr=0;
//PetscScalar *val = new PetscScalar[m*m];
...
//PetscPrintf(PETSC_COMM_WORLD, "tol [%g]\n", tol);
...
//VecView(x, PETSC_VIEWER_STDOUT_WORLD);
			...
			//val=DualHess(x);
//PetscPrintf(PETSC_COMM_WORLD, "loop [%d]\n", loop);
//=============================================== DualHess ======================================================================================================
...
//for(PetscInt i=0;i<n*m;i++){printf("tmp[%g]\n" ,tmp[i]);}

//PetscScalar *val;
//PetscMalloc1(m*m, &val);
PetscScalar *val = new PetscScalar[m*m];
	for (PetscInt i=0;i<m;i++){
...
//if (rank==2) printf( "before reduce rank[%d]\n", rank);
				//Hess[i*m+j]+=tmp[i*nloc+k]*PQ[k*m+j];
	 		  // Hess[i*m+j]+=tmp[i*nloc+k]*PQ[k*m+j];
				val[i*m+j]=val[i*m+j]+tmp[i*nloc+k]*PQ[k*m+j];	
				//printf("hess[%f] rank[%d]\n", val[i*m+j], rank);
			}

		}

	}
//for(PetscInt i=0;i<m*m;i++){ Hess[i]=val[i];};
//printf( "before reduce rank[%d]\n", rank);


	{
		PetscScalar *tmpp = new PetscScalar[m*m];
		//PetscScalar *tmpp;PetscCalloc1(m*m, &tmpp);
		for(PetscInt i=0;i<m*m;i++) {tmpp[i]=val[i];}
//printf( "before reduce rank[%d]\n", rank);
MPI_Barrier(PETSC_COMM_WORLD);
		MPI_Allreduce(val, tmpp, m*m,MPIU_SCALAR, MPI_SUM, PETSC_COMM_WORLD );
		//MPI_Bcast(&tmpp, m*m, MPIU_SCALAR, 0, PETSC_COMM_WORLD);
//for(PetscInt i=0;i<m*m;i++) {printf("tmpp[%f]\n", tmpp[i]);}

//PetscPrintf(PETSC_COMM_WORLD, "after reduce\n");
		memcpy(val,tmpp,sizeof(PetscScalar)*m*m);
		delete [] tmpp;
	}
	...
	for (PetscInt j=0;j<m;j++){
		...
		val[j*m+j]+= -mu[j]/lam[j];
	}

	if (lamai>0.0){
		for (PetscInt j=0;j<m;j++){
			for (PetscInt k=0;k<m;k++){
				val[j*m+k]+= -10.0*a[j]*a[k];
			}
		}
	}
...
//========================================================== Factorize ========================================================================================
for (PetscInt ss=0;ss<m-1;ss++){
		for (PetscInt i=ss+1;i<m;i++){
			val[i*m+ss] = val[i*m+ss] / val[ss*m+ss];
			for (PetscInt j=ss+1;j<m;j++){
				val[i*m+j] = val[i*m+j] - val[i*m+ss]*val[ss*m+j];
			}
		}
...
for (PetscInt i=1;i<m;i++){
		PetscScalar a = 0.0;
		for (PetscInt j=0;j<i;j++){
			a = a - val[i*m+j]*grad[j];
		}
		grad[i] = grad[i] + a;
	}

	grad[m-1] = grad[m-1]/val[(m-1)*m+(m-1)];

//PetscPrintf(PETSC_COMM_WORLD, "here grad[%g]\n", grad[m-1]);

	for (PetscInt i=m-2;i>=0;i--){
		PetscScalar a=grad[i];
...
for (PetscInt j=0;j<m;j++){
		...
		PetscScalar *tmp_new = new PetscScalar[2*m];
               
		for(PetscInt i=0;i<2*m;i++) {tmp_new[i]=0.0;}

		MPI_Allreduce(res, tmp_new,2*m,MPIU_SCALAR, MPI_SUM, PETSC_COMM_WORLD );

		memcpy(res,tmp_new,sizeof(PetscScalar)*2*m);
		delete [] tmp_new;
...
//VecView(x, PETSC_VIEWER_STDOUT_WORLD);
//PetscPrintf(PETSC_COMM_WORLD, "err[%g] epsi[%g]\n",err, 0.9*epsi);
//PetscPrintf(PETSC_COMM_WORLD, "epsi [%g]\n", epsi);
	VecRestoreArray(x,&xv);
	VecRestoreArrays(pij,m,&pijv);
	VecRestoreArrays(qij,m,&qijv);
	VecRestoreArray(L,&Lv);
...
//PetscPrintf(PETSC_COMM_WORLD, "epsi less than tol\n");

	return ierr;

}

PetscErrorCode MMA::XYZofLAMBDA(Vec x, PetscInt fac){
	PetscErrorCode ierr=0;
...
//PetscPrintf(PETSC_COMM_WORLD, "qov[%f]\n", p0v[i]);
		for (PetscInt j=0;j<m;j++){
			pjlam+=pijv[j][i]*lam[j];
			qjlam+=qijv[j][i]*lam[j];
//printf("i[%d]j[%d] xv[%f] fac[%d]\n", i,j, lam[j], fac);
		}
...
PetscErrorCode MMA::DualGrad(Vec x){
	...
	for (PetscInt j=0;j<m;j++){
		...
		for (PetscInt i=0;i<nloc;i++){
			...
			//#grad[0]+=pijv[j][i]/(Uv[i]-xv[i]) + qijv[j][i]/(xv[i]-Lv[i]);
...
PetscScalar val=0.0;
	for (PetscInt i=0;i<2*m;i++){
		if (nrI<PetscAbs(res[i])){
			 val=PetscAbs(res[i]);
//PetscPrintf(PETSC_COMM_WORLD, "res[%f]\n",val);
		}
	}
//PetscPrintf(PETSC_COMM_WORLD, "res[%g]\n",val);
	delete [] res;
	VecRestoreArray(x,&xv);
	VecRestoreArrays(pij,m,&pijv);
	VecRestoreArrays(qij,m,&qijv);
